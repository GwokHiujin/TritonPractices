...
def max_kernel(input_ptr, 
    ...
    output = tl.max(input, axis=(0 if dim == 0 else 1))
...
def _max(x, dim):
    ...
    max_kernel[grid](x, M, N, stride_x, stride_y, output, dim=dim, BLOCK_SIZE=BLOCK_SIZE)
    return output
...
output_torch_dim0 = torch.max(x, dim=0)[0]
output_torch_dim1 = torch.max(x, dim=1)[0]
output_triton_dim0 = _max(x, dim=0)
output_triton_dim1 = _max(x, dim=1)
